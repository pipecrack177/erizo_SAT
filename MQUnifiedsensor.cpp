...
MQUnifiedsensor::MQUnifiedsensor(String Placa, float Voltage_Resolution, float Sensor_Voltage, float Divider_Factor, int ADC_Bit_Resolution, int pin, String type) {
  this->_pin = pin;
  ...
  this->_SENSOR_VOLTAGE = Sensor_Voltage;
  this->_DIVIDER_FACTOR = Divider_Factor;
...
void MQUnifiedsensor::setSensorVoltage(float voltage)
{
  _SENSOR_VOLTAGE = voltage;
}
void MQUnifiedsensor::setDividerFactor(float factor)
{
  _DIVIDER_FACTOR = factor;
}
...
float MQUnifiedsensor::readSensor(bool isMQ303A, float correctionFactor, bool injected)
{
  ...
  _RS_Calc = ((_SENSOR_VOLTAGE * _RL) / (_sensor_volt * _DIVIDER_FACTOR)) - _RL; //Get value of RS in a gas
...
float MQUnifiedsensor::calibrate(float ratioInCleanAir) {
  ...
  */
  ...
  RS_air = ((_SENSOR_VOLTAGE * _RL) / (_sensor_volt * _DIVIDER_FACTOR)) - _RL; //Calculate RS in fresh air
...
// por si las moscas _RS_Calc = ((_VOLT_RESOLUTION*_RL)/_sensor_volt)-_RL; 